function [a , aline, fnorm , ftan, gamma, inflowangle, aoa, cl, cd] = ...
...
%     pitch - pitch angle of the blade
    ...
    for ii = 1:N    % Loop until convergence is achieved
        ...
        [fnorm, ftan, gamma, aoa, cl, cd] = loadBladeElement2(vnorm, vtan,chord,...
                                twist, polar_alpha, polar_cl, polar_cd, pitch);

        % 3D force in axial direction
        load3Daxial =fnorm*Radius*(r2_R-r1_R)*NBlades; 

        % 3D force in azimuthal/tangential direction (not used here)
        % load3Dtan =loads[1]*Radius*(r2_R-r1_R)*NBlades 
        ...
        if UsePrandtl
            [Prandtl, Prandtltip, Prandtlroot] = PrandtlTipRootCorrection(r_R, ...
