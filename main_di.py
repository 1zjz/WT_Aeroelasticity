...
def plot_combined_subplot_red_freq(y_label, ax1, ax2, ax3, ax4, x_lst, y_mat, y_qs_mat, blade_loc_id, blade_loc_tag,
                                   color, line, tag, i):
...
def plot_combined_subplot_red_freq_norm(y_label, ax1, ax2, ax3, ax4, x_lst, y_mat, y_qs_mat, blade_loc_id,
                                        blade_loc_tag, color, line, tag, i):
    # Plot once the quasi steady solution with its label
    if not i:
        ax1.plot(x_lst, y_qs_mat[:, blade_loc_id[0]], color=qs_color, linestyle='solid', label='Quasi-steady')
        ax2.plot(x_lst, y_qs_mat[:, blade_loc_id[1]], color=qs_color, linestyle='solid')
...
def plot_combined_subplot_elem(y_label, ax1, ax2, ax3, x_lst, y_mat, y_qs_mat, row_interest, model_tags, color, line,
                               line_label, qs_color, model_i, time_step_counter):
...
if __name__ == '__main__':
    ...
    for case_tag_i, case_tag in enumerate(case_tag_range):
        ...
        if case_tag == 'A2' or case_tag == 'B2':
            ...
            freq_red_range = [
                0.00]  # Dummy variable, as long as it is one value, step change responses will only be plotted once
        ...
        for case_ID_i, case_ID in enumerate(case_ID_range):
            ...
            for freq_red_index, freq_red in enumerate(freq_red_range):
                ...
                fig_a, (ax_a1, ax_a2, ax_a3, ax_a4) = plt.subplots(4, 1, sharex='all',
                                                                   figsize=(9, 5))  # a: Induction factor
                fig_ct, (ax_ct1, ax_ct2, ax_ct3, ax_ct4) = plt.subplots(4, 1, sharex='all',
                                                                        figsize=(9, 5))  # ct: Thrust coefficient
                fig_cq, (ax_cq1, ax_cq2, ax_cq3, ax_cq4) = plt.subplots(4, 1, sharex='all',
                                                                        figsize=(9, 5))  # cq: Torque coefficient
                fig_aoa, (ax_aoa1, ax_aoa2, ax_aoa3, ax_aoa4) = plt.subplots(4, 1, sharex='all', figsize=(
                9, 5))  # aoa: Angle of attack (alpha)
                fig_phi, (ax_phi1, ax_phi2, ax_phi3, ax_phi4) = plt.subplots(4, 1, sharex='all',
                                                                             figsize=(9, 5))  # phi: Inflow angle

                # Loop over each model
                for i, model in enumerate(('pp', 'lm', 'oye')):
                    print(model)
                    if case_tag == 'A1':
                        r_list, t_list, ctr, cqr, a, alpha, phi, ctr_qs, cqr_qs, a_qs, alpha_qs, phi_qs = read_data(
                            'ct', *ct_steps[case_ID - 1], None,
                            model=model)  # NB: Use case_ID-1 to comply with Python indexing convention
                    elif case_tag == 'A2':
                        r_list, t_list, ctr, cqr, a, alpha, phi, ctr_qs, cqr_qs, a_qs, alpha_qs, phi_qs = read_data(
                            'ct', *ct_sins[case_ID - 1], freq_red, model=model)
                    elif case_tag == 'B1':
                        r_list, t_list, ctr, cqr, a, alpha, phi, ctr_qs, cqr_qs, a_qs, alpha_qs, phi_qs = read_data(
                            'u_inf', *u_inf_steps[case_ID - 1], None, model=model)
                    elif case_tag == 'B2':
                        r_list, t_list, ctr, cqr, a, alpha, phi, ctr_qs, cqr_qs, a_qs, alpha_qs, phi_qs = read_data(
                            'u_inf', *u_inf_sins[case_ID - 1], freq_red, model=model)
                    else:
                        print('Warning: Invalid case tag enterred.')

                    # Assemble the plots
                    plot_combined_subplot('a [-]', ax_a1, ax_a2, ax_a3, ax_a4, t_list, a, a_qs, blade_loc_id,
                                          blade_loc_tag, model_colors[i], model_line[i], model_tag[i], i)
                    plot_combined_subplot('$C_t$ [-]', ax_ct1, ax_ct2, ax_ct3, ax_ct4, t_list, ctr, ctr_qs,
                                          blade_loc_id, blade_loc_tag, model_colors[i], model_line[i], model_tag[i], i)
                    plot_combined_subplot('$C_q$ [-]', ax_cq1, ax_cq2, ax_cq3, ax_cq4, t_list, cqr, cqr_qs,
                                          blade_loc_id, blade_loc_tag, model_colors[i], model_line[i], model_tag[i], i)
                    plot_combined_subplot('$\\alpha$ [deg]', ax_aoa1, ax_aoa2, ax_aoa3, ax_aoa4, t_list, alpha,
                                          alpha_qs, blade_loc_id, blade_loc_tag, model_colors[i], model_line[i],
                                          model_tag[i], i)
                    plot_combined_subplot('$\\phi$ [deg]', ax_phi1, ax_phi2, ax_phi3, ax_phi4, t_list, phi, phi_qs,
                                          blade_loc_id, blade_loc_tag, model_colors[i], model_line[i], model_tag[i], i)
            ...
            if case_tag == 'A2' or case_tag == 'B2':  # Loop over each model
                ...
                for i, model in enumerate(('pp', 'lm', 'oye')):
                    print(model)
                    # Initialise the plots
                    fig_a_k, (ax_ak1, ax_ak2, ax_ak3, ax_ak4) = plt.subplots(4, 1, sharex='all',
                                                                             figsize=(9, 5))  # a: Induction factor
                    fig_ct_k, (ax_ctk1, ax_ctk2, ax_ctk3, ax_ctk4) = plt.subplots(4, 1, sharex='all', figsize=(
                    9, 5))  # ct: Thrust coefficient
                    fig_cq_k, (ax_cqk1, ax_cqk2, ax_cqk3, ax_cqk4) = plt.subplots(4, 1, sharex='all', figsize=(
                    9, 5))  # cq: Torque coefficient
                    fig_aoa_k, (ax_aoak1, ax_aoak2, ax_aoak3, ax_aoak4) = plt.subplots(4, 1, sharex='all', figsize=(
                    9, 5))  # aoa: Angle of attack (alpha)
                    fig_phi_k, (ax_phik1, ax_phik2, ax_phik3, ax_phik4) = plt.subplots(4, 1, sharex='all', figsize=(
                    9, 5))  # phi: Inflow angle
                    ...
                    for freq_red_index, freq_red in enumerate(freq_red_range):  # Loop over each frequency
                        ...
                        # Assemble the plots
                        freq_red_grayscale = str(1 - (freq_red_index + 1) / len(
                            freq_red_range))  # Plot the different reduced frequency lines with a shade of grey [0 = Black; 1 = White]
                        plot_combined_subplot_red_freq('a [-]', ax_ak1, ax_ak2, ax_ak3, ax_ak4, t_list, a, a_qs,
                                                       blade_loc_id, blade_loc_tag, freq_red_grayscale, '--',
                                                       'k = ' + str(freq_red), freq_red_index)
                        plot_combined_subplot_red_freq('$C_t$ [-]', ax_ctk1, ax_ctk2, ax_ctk3, ax_ctk4, t_list, ctr,
                                                       ctr_qs, blade_loc_id, blade_loc_tag, freq_red_grayscale, '--',
                                                       'k = ' + str(freq_red), freq_red_index)
                        plot_combined_subplot_red_freq('$C_q$ [-]', ax_cqk1, ax_cqk2, ax_cqk3, ax_cqk4, t_list, cqr,
                                                       cqr_qs, blade_loc_id, blade_loc_tag, freq_red_grayscale, '--',
                                                       'k = ' + str(freq_red), freq_red_index)
                        plot_combined_subplot_red_freq('$\\alpha$ [deg]', ax_aoak1, ax_aoak2, ax_aoak3, ax_aoak4,
                                                       t_list, alpha, alpha_qs, blade_loc_id, blade_loc_tag,
                                                       freq_red_grayscale, '--', 'k = ' + str(freq_red), freq_red_index)
                        plot_combined_subplot_red_freq('$\\phi$ [deg]', ax_phik1, ax_phik2, ax_phik3, ax_phik4, t_list,
                                                       phi, phi_qs, blade_loc_id, blade_loc_tag, freq_red_grayscale,
                                                       '--', 'k = ' + str(freq_red), freq_red_index)
            ...
            if case_tag == 'A2' or case_tag == 'B2':  # Loop over each model
                ...
                for i, model in enumerate(('pp', 'lm', 'oye')):
                    ...
                    for freq_red_index, freq_red in enumerate(freq_red_range):  # Loop over each frequency
                        ...
                        freq_red_grayscale = str(1 - (freq_red_index + 1) / len(
                            freq_red_range))  # Plot the different reduced frequency lines with a shade of grey [0 = Black; 1 = White]
                        plot_combined_subplot_red_freq_norm('a [-]', ax_ak1, ax_ak2, ax_ak3, ax_ak4, norm_time, a, a_qs,
                                                            blade_loc_id, blade_loc_tag, freq_red_grayscale, '--',
                                                            'k = ' + str(freq_red), freq_red_index)
                        plot_combined_subplot_red_freq_norm('$C_t$ [-]', ax_ctk1, ax_ctk2, ax_ctk3, ax_ctk4, norm_time,
                                                            ctr, ctr_qs, blade_loc_id, blade_loc_tag,
                                                            freq_red_grayscale, '--', 'k = ' + str(freq_red),
                                                            freq_red_index)
                        plot_combined_subplot_red_freq_norm('$C_q$ [-]', ax_cqk1, ax_cqk2, ax_cqk3, ax_cqk4, norm_time,
                                                            cqr, cqr_qs, blade_loc_id, blade_loc_tag,
                                                            freq_red_grayscale, '--', 'k = ' + str(freq_red),
                                                            freq_red_index)
                        plot_combined_subplot_red_freq_norm('$\\alpha$ [deg]', ax_aoak1, ax_aoak2, ax_aoak3, ax_aoak4,
                                                            norm_time, alpha, alpha_qs, blade_loc_id, blade_loc_tag,
                                                            freq_red_grayscale, '--', 'k = ' + str(freq_red),
                                                            freq_red_index)
                        plot_combined_subplot_red_freq_norm('$\\phi$ [deg]', ax_phik1, ax_phik2, ax_phik3, ax_phik4,
                                                            norm_time, phi, phi_qs, blade_loc_id, blade_loc_tag,
                                                            freq_red_grayscale, '--', 'k = ' + str(freq_red),
                                                            freq_red_index)
            ...
            if case_tag == 'A1' or case_tag == 'B1':
                ...
                # Initialise the plots
                fig_a_elem, (ax_a1_elem, ax_a2_elem, ax_a3_elem) = plt.subplots(1, 3, sharey='all',
                                                                                figsize=(9, 5))  # a: Induction factor
                fig_ct_elem, (ax_ct1_elem, ax_ct2_elem, ax_ct3_elem) = plt.subplots(1, 3, sharey='all', figsize=(
                9, 5))  # ct: Thrust coefficient
                fig_cq_elem, (ax_cq1_elem, ax_cq2_elem, ax_cq3_elem) = plt.subplots(1, 3, sharey='all', figsize=(
                9, 5))  # cq: Torque coefficient
                fig_aoa_elem, (ax_aoa1_elem, ax_aoa2_elem, ax_aoa3_elem) = plt.subplots(1, 3, sharey='all', figsize=(
                9, 5))  # aoa: Angle of attack (alpha)
                fig_phi_elem, (ax_phi1_elem, ax_phi2_elem, ax_phi3_elem) = plt.subplots(1, 3, sharey='all', figsize=(
                9, 5))  # phi: Inflow angle
